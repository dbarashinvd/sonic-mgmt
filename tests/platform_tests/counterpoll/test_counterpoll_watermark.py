...
MAPS = 'maps'
QUEUE_MAPS = { 'prefix' :  'QUEUE'
, MAPS: ['COUNTERS_QUEUE_NAME_MAP'
...
RELEVANT_MAPS = {CounterpollConstants.QUEUE :
                     {MAPS: [QUEUE_MAPS], CounterpollConstants.TYPE : [CounterpollConstants.QUEUE_STAT_TYPE] }
, CounterpollConstants.WATERMARK :
                     {MAPS: [QUEUE_MAPS, PG_MAPS] , CounterpollConstants.TYPE :
                         [CounterpollConstants.QUEUE_WATERMARK_STAT_TYPE, CounterpollConstants.PG_WATERMARK_STAT_TYPE] }
, CounterpollConstants.PG_DROP : {MAPS: [PG_MAPS] , CounterpollConstants.TYPE :
                            [CounterpollConstants.PG_DROP_STAT_TYPE] }
}

WATERMARK_COUNTERS_DB_STATS_TYPE = [  'USER_WATERMARKS'
...
def test_counterpoll_queue_watermark_pg_drop(duthosts, localhost, enum_rand_one_per_hwsku_hostname, dut_vars):
    ...
    with allure.step("enabling randomly selected counterpoll {} on {} ...".format(duthost.hostname, [tested_counterpoll])):
        ...
        for map_to_verify in maps_to_verify:
            ...
            for line in map_output:
                try:
                    ...
                    failed = "MAP {} was not found in {} MAPS list".format(map_prefix)
    ...
    with allure.step("enable all {} counterpolls on {} and check results...".format(RELEVANT_COUNTERPOLLS, duthost.hostname)):
...
def verify_counterpoll_status(duthost, counterpoll_list, expected):
    with allure.step("verifying {} for {} in output of {} on {}..."
        ...
        for counterpoll in counterpoll_list:
            pytest_assert(expected in verified_output_dict[counterpoll]
                          , "{} is {}. expected to be {}".format(counterpoll, verified_output_dict[counterpoll], expected))
